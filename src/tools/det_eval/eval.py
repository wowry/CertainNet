...
from tools.det_eval.rotate_iou import rotate_iou_gpu_eval
...
def _prepare_data(gt_annos, dt_annos, dataset, current_class, class_to_name, difficulty):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(gt_annos[i], dt_annos[i], dataset, current_class, class_to_name, difficulty)
...
def eval_class(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, dataset, current_class, class_to_name, difficulty)
...
def get_official_eval_result(gt_annos,
    ...
    overlap_mod = [[
            0.7 if dataset == 'kitti' and v.lower() == 'car' else 0.5 for k, v in class_to_name.items()
        ] for d in range(3)]
    min_overlaps = np.stack([overlap_mod], axis=0)
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            mAPbbox_str = ", ".join(f"{v:.2f}" for v in mAPbbox)
            """
